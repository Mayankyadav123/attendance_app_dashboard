        ...
        <Row>
          ...
          <Form.Item label='Name'>
              {getFieldDecorator('name', {
                rules: [{ required: true, message: 'Please enter the PSID' }],
              })(
                <Input placeholder='Enter employee PSID'/>
              )}
            </Form.Item>
            <Form.Item label='PSID'>
              {getFieldDecorator('psid', {
                rules: [{ required: true, message: 'Please enter the PSID' }],
              })(
                <Input placeholder='Enter employee PSID'/>
              )}
...
class Product extends React.Component {
    ...
    this.state = {
      ...
      columns: [{
        ...
        render: (text, record, index) => {
          return (
            <div>
              <div style={{ color: 'grey' }}>{record.psid}</div>
            </div>
          );
        },
      },{
      ...
      },
       {
        ...
        render: (text, record, index) => {
          return (
            <div>
              <div style={{ color: 'grey' }}>{record.project}</div>
            </div>
          );
        },
      },
  ...
  fetchEmplyeeAttendance = (record) => {
    let { current } = this.state;    
    if (record._id) {
      this.setState({ loading: true });
      client.service('attendance').find({
        query: {
          'user': record._id
        }
      }).then((items) => {        
        this.setState({
          loading: false,
          attendanceData: items.data,
          refreshCal: true
        });
        setTimeout(_=>{this.setState({refreshCal:false}),2000})
      });
    }
  }
      ...
      <div>
        <Table
          ...
          onRow={(record) => {
            return {              
              onClick: () => {
                this.setState({ detail: true, current: record })
                this.fetchEmplyeeAttendance(record);
              }
            };
          }}
        />
        <Drawer
          ...
          width={720}
        >
          <Calendar attendanceData={this.state.attendanceData.map(item => item.createdAt)} refresh={this.state.refreshCal}/>
